...
test("media pipeline is deterministic, non-fatal for missing sources, and emits WebP plus PNG", async () => {
  ...
  writeFileSync(path.join(incoming, "cold-open.png"), sourcePng(32, 18));
  ...
  try {
    ...
    assert.equal(result.records.coldOpen?.available, true);
    assert.equal(result.records.sheet01?.available, true);
    assert.equal(result.records.sheet02?.available, false);
    assert.equal(result.records.trophy?.available, false);
    assert.ok(result.missing.includes("trophy.png"));
    assert.ok(result.missing.includes("sheet02.png"));
    assert.deepEqual(
      await imageDimensions(path.join(publicDirectory, "media", "cold-open.png")),
      [1080, 1920],
    );
    assert.deepEqual(
    ...
    const coldOpenWebp = readFileSync(path.join(publicDirectory, "media", "cold-open.webp"));
    assert.equal(coldOpenWebp.subarray(0, 4).toString("ascii"), "RIFF");
    assert.equal(coldOpenWebp.subarray(8, 12).toString("ascii"), "WEBP");
...
test("media pipeline keeps a usable PNG when the optional WebP encoder is unavailable", async () => {
  ...
  try {
    ...
    const coldOpen = result.records.coldOpen;
    assert.equal(coldOpen?.available, true);
    assert.ok(coldOpen?.png);
    assert.equal(coldOpen?.webp, null);
    assert.match(String(coldOpen?.reason), /FFmpeg WebP encoder unavailable/);
    assert.equal(existsSync(path.join(publicDirectory, "media", "cold-open.png")), true);
    assert.equal(existsSync(path.join(publicDirectory, "media", "cold-open.webp")), false);
...
test("media pipeline preserves encoder fallbacks and refuses real asset overwrite", async () => {
  ...
  const publicDirectory = path.join(root, "public");
  const generatedFile = path.join(root, "generated", "media.generated.ts");
  const sourceFile = path.join(incoming, "cold-open.png");
  const pngFile = path.join(publicDirectory, "media", "cold-open.png");
  const webpFile = path.join(publicDirectory, "media", "cold-open.webp");
  mkdirSync(incoming, { recursive: true });
  ...
  try {
    ...
    assert.ok(withoutEncoder.records.coldOpen?.webp);
    assert.match(
      String(withoutEncoder.records.coldOpen?.reason),
